export default {
  ...
  async reset() {
    await storeValue("cashCountRows", this.initialRows());
  },

  updateQuantity(rowIndex, value) {
    const rows = [...(appsmith.store.cashCountRows || this.initialRows())];
    ...
    storeValue("cashCountRows", rows);

    if (typeof CountedCashInput !== "undefined") {
      CountedCashInput.setValue(this.total(rows).toFixed(2));
    }
  },
